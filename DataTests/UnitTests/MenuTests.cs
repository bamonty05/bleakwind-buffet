...
namespace BleakwindBuffet.DataTests.UnitTests
{
    public class MenuTests
    {
        ...
        [InlineData("Double Drougr")]
        [InlineData("Gardon Orc Omelette")]
        ...
        {
            //Assert.Contains(Menu.Entrees, (item) => { return item.ToString().Equals(name); });

            Assert.Collection<IEnumerable<IOrderItem>>((IEnumerable<IEnumerable<IOrderItem>>)Menu.Entrees, (item) => item.ToString().Contains(name));
        }
